...
async function injectNamesAndLayout(packageInfo) {
  ...
  const namesBySource = Object.fromEntries(catalog.map(plugin => [plugin.name, buildChineseName(plugin.name, plugin.category)]));
  const expression = `(()=>{const names=${JSON.stringify(namesBySource)},categories=${JSON.stringify(categoryNames)};const telemetry=window.__codexMarketplaceZhTelemetry??={schema:1,installedAt:Date.now(),cycles:0,nameChanges:0,categoryChanges:0,descriptionChanges:0,nameObserverCallbacks:0,descriptionObserverCallbacks:0,nameRafSchedules:0,descriptionRafSchedules:0};telemetry.catalogNames=Object.keys(names).length;telemetry.catalogCategories=Object.keys(categories).length;let style=document.getElementById('codex-cn-plugin-layout');if(!style){style=document.createElement('style');style.id='codex-cn-plugin-layout';document.head.appendChild(style)}style.textContent=\`div[class~="group/plugin-row"]{display:flex!important;align-items:flex-start!important;gap:.25rem!important;min-height:6.25rem!important;height:auto!important;overflow:visible!important}div[class~="group/plugin-row"]>button{position:relative!important;display:flex!important;flex:0 0 4.75rem!important;flex-direction:column!important;align-items:center!important;justify-content:flex-start!important;gap:.35rem!important;width:4.75rem!important;min-width:4.75rem!important;height:6rem!important;padding:.25rem .125rem!important;overflow:visible!important}div[class~="group/plugin-row"]>button>span:first-child{flex:0 0 2.75rem!important;margin:0!important}div[class~="group/plugin-row"]>button>span[aria-hidden="true"]{position:static!important;display:block!important;box-sizing:border-box!important;width:100%!important;min-width:0!important;max-width:100%!important;min-height:1.75rem!important;margin:0!important;padding:0 .125rem!important;overflow:hidden!important;overflow-wrap:anywhere!important;white-space:normal!important;text-align:center!important;text-overflow:clip!important;font-size:.625rem!important;line-height:.8rem!important;opacity:1!important;transform:none!important;transition:none!important}\`;const active=()=>Boolean(document.getElementById('plugins-page-search')||document.querySelector('div[class~="group/plugin-row"]'));const apply=()=>{const result={names:0,categories:0,active:active()};if(!result.active){telemetry.lastNameResult=result;return result}for(const element of document.body.querySelectorAll('*')){if(element.children.length)continue;const displayed=element.textContent.trim();const nameSource=element.getAttribute('data-codex-cn-name-source')??displayed;const nameTarget=names[nameSource];if(nameTarget){if(element.textContent!==nameTarget){element.setAttribute('data-codex-cn-name-source',nameSource);element.textContent=nameTarget;result.names++}continue}const categorySource=element.getAttribute('data-codex-cn-category-source')??displayed;const categoryTarget=categories[categorySource];if(categoryTarget&&element.textContent!==categoryTarget){element.setAttribute('data-codex-cn-category-source',categorySource);element.textContent=categoryTarget;result.categories++}}telemetry.cycles++;telemetry.nameChanges+=result.names;telemetry.categoryChanges+=result.categories;telemetry.lastNameResult={...result,at:Date.now()};return result};window.__codexCnStableObserver?.disconnect?.();let scheduled=false;const observer=new MutationObserver(()=>{telemetry.nameObserverCallbacks++;if(scheduled)return;scheduled=true;telemetry.nameRafSchedules++;requestAnimationFrame(()=>{scheduled=false;apply()})});observer.observe(document.documentElement,{childList:true,subtree:true,characterData:true});window.__codexCnStableObserver=observer;const initial=apply();return{mapped:Object.keys(names).length,categories:Object.keys(categories).length,initial,telemetry:{cycles:telemetry.cycles,nameChanges:telemetry.nameChanges,categoryChanges:telemetry.categoryChanges}}})()`;
...
async function injectCurrentVersion(packageInfo) {
  ...
  const expression = [
    ...
    "const telemetry=window.__codexMarketplaceZhTelemetry??={schema:1,installedAt:Date.now(),cycles:0,nameChanges:0,categoryChanges:0,descriptionChanges:0,nameObserverCallbacks:0,descriptionObserverCallbacks:0,nameRafSchedules:0,descriptionRafSchedules:0};telemetry.catalogDescriptions=Object.keys(descriptions).length;",
    'const active=()=>Boolean(document.getElementById("plugins-page-search")||document.querySelector("div[class~=\\"group/plugin-row\\"]"));',
    "const apply=()=>{const result={changed:0,active:active()};if(!result.active){telemetry.lastDescriptionResult=result;return result}for(const element of document.body.querySelectorAll('*')){if(element.children.length)continue;const saved=element.getAttribute('data-codex-cn-description-source');const source=saved??element.textContent.trim();if(!source||hasHan(source))continue;let target=descriptions[source];if(!target&&/[A-Za-z]/.test(source)){const prefix=source.replace(/(?:…|\.\.\.)$/,'');if(prefix.length>5){const key=Object.keys(descriptions).find(value=>value.startsWith(prefix));target=key?descriptions[key]:undefined;}}if(!target||target===source||element.textContent===target)continue;element.setAttribute('data-codex-cn-description-source',source);element.textContent=target;result.changed++;}telemetry.cycles++;telemetry.descriptionChanges+=result.changed;telemetry.lastDescriptionResult={...result,at:Date.now()};return result;};",
    "window.__codexMarketplaceZhDescriptionObserver?.disconnect?.();let queued=false;const observer=new MutationObserver(()=>{telemetry.descriptionObserverCallbacks++;if(queued)return;queued=true;telemetry.descriptionRafSchedules++;requestAnimationFrame(()=>{queued=false;apply();});});observer.observe(document.documentElement,{childList:true,subtree:true,characterData:true});window.__codexMarketplaceZhDescriptionObserver=observer;const initial=apply();return{descriptions:Object.keys(descriptions).length,initial,telemetry:{cycles:telemetry.cycles,descriptionChanges:telemetry.descriptionChanges}};",
    "})()",
  ].join("");
  await evaluate(page.webSocketDebuggerUrl, expression);
  return true;
